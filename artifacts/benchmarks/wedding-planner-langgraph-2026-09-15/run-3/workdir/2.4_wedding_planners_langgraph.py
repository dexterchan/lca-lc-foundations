...
#
# main() has a 110-second total timeout and returns a partial plan on timeout.
...
class WeddingPlanner:
    ...
    async def ainvoke(self, message: str, *, session_id: str,
                      timeout_seconds: float = 110) -> WeddingState:
...
def create_live_workers():
    ...
    def specialist(tools, schema, instructions, *, context_schema=None):
        return create_agent(
            ...
            system_prompt=(
                instructions
                + "\nUse the provided tools. Report success=false on errors or no usable "
                "results. Never invent findings. Include source URLs or database facts "
                ...
                "your final structured answer; report success=false if they are "
                "insufficient. Do not call that tool again. "
                "The success field must be a JSON boolean (true or false), never "
                "a quoted string. If your final answer fails validation, correct "
    ...
    venue_agent = specialist(
        [web_search], VenueResult,
        "Choose ONE real wedding venue in the requested destination with room for "
        "the guest count. Give its name and a nearby arrival airport's three-letter "
        "code. Distinguish advertised capacity from confirmed date availability. "
    ...
    async def find_venue(state):
        ...
        return await ask_specialist(
            venue_agent, prompt, VenueResult, {"web_search"},
...
async def main(timeout_seconds: float = 110, *, message: str | None = None,
