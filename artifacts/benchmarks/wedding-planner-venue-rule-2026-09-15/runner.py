...
def run_trials(args):
    ...
    config_module = load_module("wedding_benchmark_defaults", SOURCE)
    planner_limit = inspect.signature(config_module.main).parameters["timeout_seconds"].default
    step_limit = inspect.signature(config_module.build_wedding_graph).parameters["step_timeout"].default
    if args.timeout is None:
        args.timeout = planner_limit + 30
    ...
    manifest = {
        ...
        "planner_timeout_s": planner_limit, "step_timeout_s": step_limit,
    ...
    for number in range(1, args.runs + 1):
        ...
        print(f"Run {number}/{args.runs} started; planner limit {planner_limit:g}s.", flush=True)
        started = time.perf_counter()
